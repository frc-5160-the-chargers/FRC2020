...
class Drivetrain:
    ...
    def turn_to_angle(self, angle, next = None, global_angle = False):
        if self.state != DrivetrainState.PID_TURNING or (angle + (self.navx.get_heading() if not global_angle else 0)) != self.turn_pid.get_target():
            self.pid_manager.stop_controllers()
            self.powertrain.reset_state();
            self.state = DrivetrainState.PID_TURNING
            self.turn_pid.run_setpoint(angle + (self.navx.get_heading() if not global_angle else 0))
            self.callback = next
